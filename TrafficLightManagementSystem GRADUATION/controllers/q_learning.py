...
This controller implements a tabular Q-Learning approach to control
traffic light phases based on queue lengths and waiting times.
"""
...
class QLearningAgent:
    ...
    def __init__(self, tl_id, phases, controlled_lanes, alpha=0.5, gamma=0.95, epsilon=0.1):
        """
    ...
    def get_state(self):
        ...
        For simplicity, we discretize queue lengths into bins.
        """
        queue_lengths = []
...
def train(sumo_cfg, start_time, end_time, num_episodes, save_dir="models/q_learning", gui=False, progress_callback=None):
    """
    Train Q-learning agents for traffic light control.
    ...
    for episode in range(num_episodes):
        ...
        # Create or load Q-learning agents for each traffic light
        agents = {}
        for tl_id in tl_ids:
            phases, controlled_lanes = get_tl_info(tl_id)
            agent = QLearningAgent(tl_id, phases, controlled_lanes)
...
def run(sumo_cfg, start_time, end_time, gui=False, model_dir="models/q_learning", training_mode=False):
    """
    Run the Q-learning traffic light controller simulation.
    ...
    # Create Q-learning agents for each traffic light
    agents = {}
    for tl_id in tl_ids:
        phases, controlled_lanes = get_tl_info(tl_id)
        agent = QLearningAgent(tl_id, phases, controlled_lanes)
